...
export default async function ArtuneCaseStudyPage({
  params,
}: {
      ...
      <main className="flex-grow bg-[#0c0d14] text-slate-100 overflow-hidden font-sans">
        ...
        <section className="relative pt-24 pb-16 md:pt-32 md:pb-24">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
            {/* Main Title */}
            <h1 className="text-3xl sm:text-5xl lg:text-6xl font-semibold text-white mb-4 leading-tight tracking-wide">
              {cs.title}
            </h1>

            {/* Glowing Subtitle */}
            <p className="text-base sm:text-lg font-normal text-slate-200 mb-12 tracking-wide select-none max-w-4xl mx-auto">
              {cs.subtitle.split(/(Artune)/).map((part: string, idx: number) => 
                part === "Artune" ? (
                  <span key={idx} className="text-[#2ebf9b] font-medium">{part}</span>
                ) : (
                  part
                )
              )}
            ...
            {/* Hero Banner HTML Implementation (Artune) */}
            <div className="relative w-full max-w-6xl mx-auto rounded-3xl overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.5)] bg-[#6B4EFF] flex flex-col md:flex-row items-center px-4 md:px-12 h-[350px] md:h-[450px]">
              
              {/* Yellow Shapes */}
              <div className="absolute -top-12 -left-12 w-40 h-40 rounded-full border-[24px] border-[#ffd000] z-0"></div>
              <div className="absolute -bottom-20 -right-10 w-48 h-48 rounded-full border-[32px] border-[#ffd000] z-0"></div>
              <div className="absolute -bottom-24 -right-2 w-4 h-56 bg-[#6B4EFF] rotate-45 z-0"></div>

              {/* Left Content: Logo */}
              <div className="relative z-10 w-full md:w-1/4 flex justify-center md:justify-start mt-8 md:mt-0">
                <span className="text-4xl md:text-5xl lg:text-6xl font-black text-white tracking-widest uppercase transform -skew-x-6 drop-shadow-md">ARTUNE</span>
              </div>

              {/* Center Content: Mockups */}
              <div className="relative z-10 w-full md:w-2/4 h-full flex justify-center items-center">
                 <div className="relative w-[140%] h-[140%] flex justify-center items-center transform scale-[0.8] md:scale-100 translate-y-6 md:translate-y-10">
                   <Image src="/images/artune/browser_mockup.png" alt="Artune Mockups" fill className="object-contain" priority />
                 </div>
              </div>

              {/* Right Content: Text */}
              <div className="relative z-10 w-full md:w-1/4 flex flex-col justify-center items-center md:items-end text-center md:text-right mb-8 md:mb-0">
                <span className="text-2xl md:text-3xl font-bold text-white leading-tight">Flat Is Dead.</span>
                <span className="text-2xl md:text-3xl font-bold text-[#ffd000] leading-tight mt-1">Depth Is Here.</span>
              </div>
            </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
              ...
              <div className="lg:col-span-6 flex justify-center items-center relative">
                {/* Glow backdrop */}
                <div className="absolute w-[300px] h-[300px] bg-[#6B4EFF]/20 rounded-full blur-[80px] pointer-events-none"></div>
                
                <div className="relative w-full max-w-[480px] aspect-[4/3] rounded-3xl overflow-hidden border border-white/10 shadow-2xl">
                  <Image
                    src="/images/artune/overview.png"
                    alt="Artune Platform Overview"
                    fill
                    className="object-cover hover:scale-105 transition-transform duration-700"
                  />
                  
                  {/* Premium floating ribbon badge */}
                  <div className="absolute top-4 right-4 bg-gradient-to-r from-[#6B4EFF] to-indigo-600 text-white text-xs font-black tracking-wider uppercase px-4 py-2 rounded-xl shadow-lg border border-white/10 flex items-center gap-1.5">
                    <span>★</span>
                    <span>Top Art Marketplace</span>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white py-10">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-center">
                ...
                {/* Right: Challenge photo */}
                <div className="lg:col-span-6 flex justify-center items-center relative">
                  <div className="relative w-full max-w-[480px] aspect-[4/3] rounded-3xl overflow-hidden border border-slate-100 shadow-xl">
                    <Image
                      src="/images/artune/challenge.png"
                      alt="Artune Backend Scaling Challenge"
                      fill
                      className="object-cover hover:scale-105 transition-transform duration-700"
                    />
                    {/* Code overlay */}
                    <div className="absolute bottom-4 left-4 bg-slate-950/85 border border-[#6B4EFF]/30 rounded-xl p-3 font-mono text-[9px] sm:text-[10px] text-teal-400 shadow-xl max-w-[240px] leading-relaxed hidden sm:block">
                      <span className="text-slate-500">// Scalable inventory system</span><br />
                      <span className="text-purple-400">const</span> <span className="text-blue-400">drop</span> = <span className="text-purple-400">await</span> queue.process(checkout);<br />
                      <span className="text-purple-400">return</span> drop.status;
                    </div>
                  </div>
                </div>
        ...
        <section className="w-full bg-white py-10 relative border-t border-slate-100">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-center">
              ...
              {/* Left Column: Solution Photo */}
              <div className="lg:col-span-6 flex justify-center items-center relative">
                <div className="relative w-full max-w-[480px] aspect-[4/3] rounded-3xl overflow-hidden border border-slate-100 shadow-xl">
                  <Image
                    src="/images/artune/solution.png"
                    alt="Artune team celebrating success"
                    fill
                    className="object-cover hover:scale-105 transition-transform duration-700"
                  />
                </div>
        ...
        <section className="w-full bg-[#161a23] py-16 relative overflow-hidden border-t border-slate-800/50">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 relative z-10 flex flex-col items-center text-center">
            {/* Artune Logo */}
            <div className="mb-8 select-none flex justify-center items-center">
               <span className="text-4xl font-black text-white tracking-widest uppercase transform -skew-x-6 drop-shadow-lg">ARTUNE</span>
            </div>
